...
{\em Observational studies} can be used to draw causal inference without controlled experiments  \cite{rosenbaum2002observational,Rubin2005,PearlBook2000\ignore{,Spirtes:book01}}.

\ignore{Social scientists and economists have been using observational data for policy development for over a century.}
\ignore{The world has rightfully latched onto the promise of ``big data".}
\ignore{discovered the potential of Big Data,
which is observational in nature.}
\ignore{
...
\ignore{Thus, differentiating between causal and predictive analysis and developing scalable techniques to draw causal conclusions from observational data is a key toward getting sustainable insights from Big Data. }\ignore{ in decision support systems.}
\ignore{
...
Computational, physical, and social scientists all increasingly want to perform causal inference on big data from observational studies: 
...
Unfortunately, current software for processing observational data for causal inference does not scale. R, Stata, SAS, and SPSS have packages such as {\em MatchIt} and {\em CEM}\cite{ho2005,iacus2009cem}, but they are designed for single-table data and are cumbersome with large datasets. For example, we found performing CEM on a dataset with 5M entries takes up to an hour using Stata, R or SAS.
...
Tempm< 0$) and input SQL for custom discretization methods.}
    \vspace{-0.33cm} \item specifying a subset of data that is relevant to the analysis.  We select five US airports  with high rate of weather-related delay, namely,
  ...
  e.g., patients in a clinical trial -- are called units. In our case, each flight
   ...
   \ignore{\dans{this part is good.
    ...
    (Add a footnote saying that records are called units in causal analysis).
     ...
     $$\E[ \text{Depdelay} | \text{LowPressure}=1 ]$$
     $$- \E[ \text{DepDelay} | \text{LowPressure}=0 ]$$
...
have:  $$ATE = \E[Y(1)|T=1]-\E[Y(0)|T=0].$$

\GSQL\  is designed for drawing causal inference from {\em
...
following weaker assumption called {\em Strong
  Ignorability}: for all $x$ the following hold:
  (1) unconfoundedness: $({Y(0), Y(1) \bigCI T} | X=x)$ and
  (2) overlap: $0 < \textrm{Pr}(T = 1 | X=x) < 1$ ~\cite{Rubin1983b}.
...
If strong ignorability holds, one can estimate ATE by
 taking the  average difference for each group with $X=x$ \ignore{\danp{Need to define
