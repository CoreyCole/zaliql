...
We are developing \GSQL, which is aimed at
...
\ignore{Neyman-Rubin Causal Model (NRCM) (also referred to as} the
...
for each unit we only know either $Y(1)$ or $Y(0)$ but not both. Thus, further assumption need for estimating ATE. \ignore{
...
In observation data the mechanism used to assign treatments
...
winter, and similarly for $T,Y(1)$.  \ignore{The vast majority of datasets
...
following weaker assumption~\cite{Rubin1983b} called {\em Strong Ignorability}: Forall $x$ the following hold:   (1) Unconfoundedness $({Y(0), Y(1) \bigCI T} | X=x)$ and
...
undefined. If strong ignorability holds one can estimate ATE by taking the aggregating the average difference for each group i.e., $E_x[E[Y(1)-Y(0)| X=x]]$.
...
average from the database leads to a large sampling error. A solution adopted in statistics is {\em matching and subclassification}~\cite{Rubin1983b}.  The idea is to match each treated unit with one or multiple control
...
\label{sec:algo}
 In the demonstration walk through the state-of-the-art matching and subclassification methods.
 For each method we show how to capture it in SQL. In addition we outline the set of techniques
 that ousted by \GSQL\ to perform them effetely. In particular we demonstrate the following methods:

{\bf Nearest Neighbor Matching}
\label{sec:nnm}
The most common matching method is that of $k:1$ nearest neighbor
matching (NNM) one of a distance metrics control matches for each treated
...
calipers (see e.g., \cite{lunt2014selecting}).} The basic SQL statement to perform NNM is depicted in Figure \ref{fig:nnmnr}. However, \GSQL\  uses more efficient query for this task, leverage from recent developments in the context of spatial-databases (see, e.g.,
\cite{obe2015postgis}). 
...
{\it Subclassification based on the propensity score} We describe the
SQL implementation of subclassification based on $n$ quintiles of the
propensity score in Figure \ref{fig:subpr}. \GSQL learn the $ps$ the propensity score of each unit by calling logistic
...
automatic discretization algorithm. Then all units with similar coarsened covariates values are placed in unique subclasses. All
subclasses with at least one treated and one control unit are retained
and the rest of units are discarded.  Let $\ccv$ be the coarsened version of $\cv$ and $R^c$ is be a discretize version of R. As depicted in Figure \ref{fig:cem}, CEM is essentially a GROUP-BY-HAVING query which is relatively expensive for example it takes more than hours to perform CEM on flight data. \GSQL\ leverage techniques from data management including pushing aggregate down to the base relation, data cube-aggregation, preprocessing wrt. multiple queries and bitmap indices.  
