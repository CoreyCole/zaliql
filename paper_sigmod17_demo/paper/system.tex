...
an interface ... % statistical validity, rigor, ...
...
problem \cite{RosenbaumRubin1983}.In a {\em randomized (or controlled) experiments} the treatments are
assigned randomly to the units; for example, in medical trials each
subject is randomly assigned the treatment or a placebo.  Randomized
experiments are the gold standard for causal analysis, and they
satisfy the following {\em independence assumption}: $\indep(\jout,T)$,
where $\jout$ denotes the joint variables $(Y(1),Y(0))$.  This implies
$E[Y(1)]=E[Y(1)|T=1]$ and similarly $E[Y(0)]=E[Y(0)|T=0]$ and we have:
%
%\vspace{-0.5cm}
\begin{eqnarray}
...
In order to derive any causal relationship in observational data one
must make the {\em unconfoundendess assumption}, which states that the
potential outcomes and the treatment assignment are conditionally
independent given the covariates $X$, \ie, $\indep(\jout, T, X)$~\cite{RosenbaumRubin1983}. It essentially means one can assume that within each group defined by
different values of $X=x$, a randomized experiment is carried out (we
discuss in next thrust the challenge of how to select the {\em
  confounding variables})
% $X$ that ensure unconfoundedness.
In
addition, one also requires the {\em overlap} assumption, which states
that at least one treated and one control unit must exist each group
(otherwise one of the outcomes $Y(0)$ or $Y(1)$ is undefined for
$X=x$). Together, these two assumptions are called the {\em Strongly
  Ignorable Treatment Assumption} or {\em SITA}, and they enable us
(in theory!) to estimate the average treatment effect separately for
each group, then aggregating over all the groups:
\begin{equation}
ATE ~~=~~ E_x[E[Y(1)-Y(0)| X=x]] ~~ =~~ E_x[E[Y(1)|T=1,X=x]] \newline -E_x[E[Y(0)|T=0, X=x]]\label{eq:oatt}
\end{equation}
...
\subsection{Subclassification and matching algorithms}
 ...
 We demonstrate the following matching and subclassification algorithms.
