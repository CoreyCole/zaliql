...
an interface ... \babak{corey will add something up here} % statistical validity, rigor, ...
...
In this project, however, we are interested in causal analysis in {\em
...
\begin{description}
\item[Strong Ignorability:] Forall $x$ the following hold:   (1) Unconfoundedness $({Y(0), Y(1) \bigCI T} | X=x)$ and
...
commonly used distance functions are presented  in Table \ref{fig:metrics}. The efficacy
...
calipers (see e.g., \cite{lunt2014selecting}).} The basic SQL statement to perform NNM is depicted in Figure \ref{fig:nnmnr}. \GSQL\  supports more sofisticated techniques for performing NNM leverage 
from recent developments in the context of spatial-databases (see, e.g.,
\cite{obe2015postgis}. We should mention that \GSQL\ uses the Madlib library develop for PostgresSQL to estimate propensity score. \babak{refrence})
