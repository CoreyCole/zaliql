...
Many tools perform causal inference
 using statistical software such as the R project. \dans{give examples and citations here.  We need to make a strong case that today's causal analysis tools do not scale} However, these toolkits do not scale to large datasets. For example, performing causal inference  on a dataset with 10M eateries \dans{eateries or entries?} takes several hours.
