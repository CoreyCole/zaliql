...
Many toolkits have been developed for performing causal inference that relies on statistical software such as R, e.g., MatchIt and CEM \cite{ho2005,iacus2009cem}. \dans{give examples and citations here.  We need to make a strong case that today's causal analysis tools do not scale} However, these toolkits do not scale to large datasets. For example, performing causal inference  on a dataset with 10M entries \dans{eateries or entries?} takes several hours using MatchIt or CEM.
