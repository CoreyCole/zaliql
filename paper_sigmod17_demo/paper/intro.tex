...
Many tools perform causal inference
 using statistical software such as R project. However, in this demonstration we show that these toolkits do not scale to large datasets.  
 Additionally, in many of the most interesting Big Data settings, the data
is highly relational (e.g, social networks, biological networks, sensor networks and more). Thus, the data is likely to pour into SQL systems. A rich ecosystem of tools and organizational requirements further encourage SQL database storage. Transferring
...
inference and runs at scale within a database engine.   \GSQL\ is designed and implemented as an extension for PostgreSQL object-relational database
systems. \GSQL\ includes a series of optimization techniques that allow it to
...
These are relatively large data sets for causal inference that can not be handled by existing tools.
\end{example}
\ignore{
...
When we make predictive analyses, whether we predict $\E[Y | X = x]$ or $\textrm{Pr}(Y | X = x)$ or
...
In other words, in causal analysis we are interested in an {\em  interventional} conditional distribution,
  i.e., the distribution obtained by (hypothetically) enforcing $X = x$ uniformly over the population.
...
\begin{figure*}[t]
  \begin{subfigure}{0.5\linewidth}
    \centering
    \hspace{-.6cm}
    \includegraphics[scale=0.14]{Figures/Setup.png}
    \caption{\scriptsize }
    \label{sfig:testaa}
  \end{subfigure}\hfill
  \begin{subfigure}{0.5\linewidth}
    \centering
    \hspace{-.6cm}
    \includegraphics[scale=0.14]{Figures/Matching-Summary.png}
    \caption{\scriptsize }
    \label{sfig:testbb}
  \end{subfigure}\hfill
\hfill
...
It is known that causal inference is possible under careful \emph{randomized} and \emph{controlled} experiments, in which a treatment (the potential cause) is assigned randomly to subjects. However, most of the big data available today are observational, meaning they are collected opportunistically. The major problem in causal inference from observational data is the existence of the so-called {\em confounding variables} which makes the direct comparison between the group that received treatment and the group that are subject to control impossible. The following example explains this issue.
...
The attendees will select a causal question by specifying a treatment,
  an outcome and particular methods to perform causal inference.
The attendee will also be able to adjust the tunable parameters for these algorithms. The demonstration will show our web application interfacing with our demo flight and weather database.

The demonstration will make three contributions. It will
  (1) explain why machine leaning methods fail to distinguish causation and correlation,
  (2) explain and demonstrate the pipeline of drawing valid causal conclusions from data, and
  (3) enable the attendees to experience different methods for causal inference and investigate their pros and cons.
