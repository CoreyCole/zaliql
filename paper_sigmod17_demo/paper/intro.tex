...
Analyzing performance data presents several challenges:% among them:
\begin{itemize}
\item High dimensionality: the data consists of tens of thousands of
  ...
  % In run tens of thousands of experiments on
  %   different systems with multiple configuration settings.  On every
  %   system, thousands of different queries are executed, where each query
  %   has a large set of features extracted from logical relational algebra
  %   operators and properties of the input and output data, as well as
  %   additional features extracted from system-specific query plans, such as
  %   which operators are used and how jobs are scheduled. We monitor roughly
  %   60 low-level systems metrics, sampled at one second granularity at each
  %   compute node for the duration of every query execution, resulting in
  %   dozens of variable length time series associated with each data record.
  %   It goes without saying, that a researcher presented with data can
  %   quickly become overwhelmed.
% \item Evolving set of features. The system must be able to allow
%   experimenters to gradually increase the number of variables as new
%   questions are raised.

    % Given the extremely large space
    % of potential variables to tune in running experiments, it is
    % impossible to collect them all. As researchers, we have to
    % decide on an initial set of parameters to vary and then gradually
    % increase the number of variables. It is important to
    % learn from each round of experiments to guide what knobs
    % to turn next, and these insights often lead to the addition of
    % variables that were observed from the start. Having the flexibility
    % to add and remove features without invalidating prior
    % experiments is crucial and helps avoid a potential rabbit hole
    % situation where making changes and additions is considered
    % too much work.
% \item Missing data. Many features apply to some experiments but
%     not others: some queries may not be supported in all systems
%     and some metrics may not be available in all deployments.
%     % One way to deal with this is to limit experiments to a set
%     % of features that are common to all systems. However, this
%     % reduces the input domain, excludes potentially valuable information,
%     % and may therefor limit discovery.
% \item Dirty data: systems don't always produce repeatable results,
%   for a variety of reasons. While transient anomalies can be
%   interesting, they must be identified as such rather than used to
%   make conclusions about intrinsic properties of systems.
  
  % Failures and stragglers occur,
  %   sporadic resource contention can cause unusually high
  %   latency, a garbage collector may interfere with normal operations,
  %   multi-tenancy issues can arise, to name a few. While
  %   these anomalies can be interesting to inspect, it is important
  %   to identify them as such and take that into account when
  %   making conclusions about performance characteristics that
  %   are intrinsic to the system, rather than one-off occurrences.
\item Unclear relationship of variables. Variables in this data
    ...
    % with each other. Determining
    % these (in)dependencies can be extremely valuable
    % in providing insight into what features are relevant to certain
    % performance properties. For example, if we can determine
    % that the metric that measures time spent in kernel space is
    % causally related to whether a system is faster or slower at
    % table scans, we can pinpoint a design feature (in this case,
    % bypassing the kernel during file I/O) as the deciding factor
    % for that operation, without any familiarity or instrumentation
    % of system internals. 
  \item Evolving research questions. The system must support
    ...
    % a clear research question to ask of the data. This is especially
    % true during debugging or when evaluating a specific change
    % in a system. While asking specific questions is an important
    % step in any data analysis, a dataset such as this can contain
    % mountains of information that can easily go undiscovered if
    % someone does not think of the right questions to ask. There is
    % a clear need to allowing the user to re-evaluate and re-iterate
    % different hypotheses easily and effectively
  \end{itemize}
