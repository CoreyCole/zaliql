...
\caption{Matching observational data for Causal inference \babak{causal hypothesis, Data-> Pruning data by matching-> Checking Balance-> Conduct Analysis please lable the arrow from balance checking to analysis with "Balanced mated data" and the arrow from balance to matching as "imbalanced matched data" non need for the "bad match" node }}
\label{fig:flowchart}
\vspace{-0.3cm}
\end{figure}
...
distributions of the confounding variables (a.k.a covariates) in the groups are more similar. A typical workfellow of causal inference via matching is shown in Figure \ref{fig:flowchart}. As depicted causal inference from observational data includes an iterative process of matching and balance checking until a satisfactory matched subset is obtained (See \ref{IacKinPor09}) for more detailed discussion). Thus developing efficient methods for performing various matching methods
is crucial to scale causal inference to large observational data.
