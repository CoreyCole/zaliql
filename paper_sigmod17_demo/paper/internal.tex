...
\ignore{Mahalanobis Distance (MD) & $(x_i-x_j)'\Sigma^{-1} (x_i-x_j)$ & where $\Sigma=$ covariance matrix \\ \hline
  ...
  \label{fig:metrics}
\end{figure}If strong ignorability holds, one can estimate ATE by
...
ATE conditioned on some function of the a set of covariates $X$. 
 \GSQL\ implements the following two matching methods:
...
he aggregate expression may be replaced by NOT EXISTS.} Specifically, all potential matches and their distances are identified by
...
{\bf Subclassification.}
>>>>>>> 38561b76429d300c96baca7fc4e6897bbffe6626
\label{sec:sub}
...
CEM is essentially a GROUP-BY-HAVING query. 
 \GSQL\  also supports several supervised and unsupervised methods for coarsening proposed in \ref{dougherty1995supervised}.
...
matching techniques, ATE can computed from the matched data. 
\GSQL\  supports a wide range of statistical tests such as t-test, Chi-Square test
to compute the statical significance of the results.
