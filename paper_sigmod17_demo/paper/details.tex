...
generates a hypothesis, pre-processes the data with a matching  method (explained below), and finally
...
We demonstrate \GSQL\ by showing several real causal analyses performed on integrated flight and weather datasets. \ignore{on two,
...
It contains historical weather data for related to flights. It includes Code (Airport ID),
...
Tempm< 0$) and input SQL for custom discretization methods.} and (3) specifying a subset of data that is relevant to the analysis.  We select five US airports  with high rate of weather-related delay, namely, San Francisco (SFO) , John F. Kennedy (JFK), Newark Liberty (EWR), George Bush (IAH), and LaGuardia Airport (LGA).
 ...
 A common measure to to compute causal effect is {\em average treatment effect} (ATE).
...
In order to draw valid causal conclusions, we need to adjust for confounding influences.
 ...
 First, we partition that data into groups with similar values of that confounding variable. 
 Then, we compute the ATE as the weighted average of the effects in each group. 
 However, real-world causal inference requires many more continuous confounding variables.
...
deemed to confound the treatment and DepDelay (Figure \ref{sfig:demo-tutorial}(d)) and
...
\GSQL\ will eventually run through 100M entries in less than a minute, whereas for CEM, in R, SPSS or SAS,
