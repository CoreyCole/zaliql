...
We demonstrate \GSQL\ by showing several real causal analysis performed on the following data: \ignore{on two,
...
\end{example}
 ...
 on flight departure delay. For example, we show that \%10 flights were delayed
(flight with more than 15min delay considered as delayed) when pressure is low; but only \%0.4 flight delayed
when pressure is high. This would suggest strong association between low-pressure on flight departure delay.
However, when examining departure delay, by airports, airline, other weather features, the flight delay-upon
low pressure-declined to less than \%1 in all groups. \footnote{This phenomenon
...
} So does low pressure affect flight departure delay?

 

 {\bf Causal questions:} In this step we the following {\em causal questions}:
...
as shown in Fig. \ref{sfig:demo-tutorial}(a).
  \item specifying a set of binary treatments (causes) that might affect DepDelay,
...
Tempm< 0$) and input SQL for custom discretization methods.} and (3) specifying a subset of data that is relevant to the analysis.  We select five US airports  with high rate of weather-related delay, namely, San Francisco (SFO) , John F. Kennedy (JFK), Newark Liberty (EWR), George Bush (IAH), and LaGuardia Airport (LGA).
 ...
 { \bf Computing ATE}: In causal inference usually the objective is to quantify
...
\ignore{\dans{For the next sentence, can you refer to Fig. 3.
  There the user has selected the treatment visibility (Visim)
   and the outcome is DepDelayMinutes.  (Acually, can you update
    ...
    the ATE for LowPressure on DepDelay, the result is  (give concrete number).}}  We show that for LowPressure, ATE is 10mins, which is a relatively large that suggests pressure affects DepDelay. However, it is known that
...
\ignore{ \dans{Use this as an excuse to continue the mini-tutorial.  The first 1-2 sentences should be general, saying that  causal analysis conditions on confounding variables. Then you immediately make it concrete and say that, if the confounding variable
 ...
 {\bf Confounding variables:} The issue is the exitance of {\em confounding variables} that  make it difficult to establish a clear causal link between treatment and outcome.   We show that the observed positive effect of LowPressure
 was the result of the confounding influence of other factors such as low visibility, snow and thunder
...
In other to draw valid causal conclusion we need to adjust for variables the confounding influences.
 ...
 computing the ATE as the weighted average of the effects in each groups. 
 
 
 However, real-world causal analysis requires a dozen or more continues confounding 
 ...
 implements {\em coarsen exact matching} and {\it propensity score matching} \cite{Rubin1983b,IacKinPor09,rosenbaum1984reducing}.
These are popular methods for covariate adjustment developed in social
science and statistics. The key goal of matching is to prune data so that
...
\begin{enumerate}
  ...
  \item we select a matching method and adjust its tuning parameters (Fig. \ref{sfig:demo-tutorial}(e)).
\end{enumerate}
...
(1)  we compare the distribution for each covariate between the
 ...
 numerical and visual summaries such as the mean difference and quantile-quantile plots Fig. \ref{sfig:demo-tutorial}(d,e).
...
For Q3, we report the major causes of flight delay at the airports under the study are
     actually different. We validate the significance of our finding with statistical test such as t-test.
...
\GSQL\ is able to run through 105M entries in a couple of minutes, whereas statistical software
 takes over 20 minutes on 4M entries. \footnote{We note that \GSQL\ is recognised as a scalable SQL-based 
 counterpart of coarsen exact matching by its inventors (See, \url{http://gking.harvard.edu/cem}).}
