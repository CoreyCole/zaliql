...
%\dans{For the next sentence, can you refer to Fig. 3.  There the user
%has selected the treatment visibility (Visim) and the outcome is
%DepDelayMinutes.  (Acually, can you update the figure so that the
%treatment is LowPressure).  If we compute the ATE for LowPressure on
%DepDelay, the result is  (give concrete number).}
...
treated group is much less \danp{higher, right?} than
...
% \dans{Use this as an excuse to continue the mini-tutorial.  The first 1-2 setnences should be general, saying that  causal analysis conditions on confounding variables. Then you immediately make it concrete and say that, if the confounding variable is LowVisibility, then the ATE for LowPressure on DepDelay decreases significantly.}
 %  \paragraph{\bf Confounding variables:}
 %  The main issue with causal inference from observational data
...
Finally, we demonstrate system scalability by performing these queries
on large data sets. As seen in Figure \ref{sfig:demo-tutorial}(g),
\GSQL is able to run queries on 4M rows in less than a minute, whereas
