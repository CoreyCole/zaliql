\corey{
  describe lalonde data to show that it is used to compare matching methods
  go to CEM paper to see what binning methods they use


  NNMWR (make pseudo)
  join only treated with control units (no duplicates to provide perfect match)
  difference between propensity scores


  with replacement is for using same control to match with multiple treated subjects
...
\babak{add description of the Lalonde data here}
...
\babak{Quality comparison with Lalonde should be place here}


% \subsubsection{Causal questions and covariate selection}
% \label{sec:def}

\label{Forget about DAGs and covarite selection}
...
\babak{add causal question regarding Lalonde here}
...
\babak{add set of covariates selected for each causal question}
